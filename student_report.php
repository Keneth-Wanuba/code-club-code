<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Student Reports</title>
    <link rel="stylesheet" href="css/styles.css">
    <link rel="stylesheet" href="css/font-awesome-4.7.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="css/font-awesome-4.7.0/css/font-awesome.css">
</head>
<body>

    <header>
        <div class="logos">
            <img src="imgs/Logos/Code Academy logo-15.png" alt="Your Logo" width="100" height="100">
            <h>WCIS</h>
        </div>
        <nav class="navigation">
           
                <li><a href="index.php">Home</a></li>
                <li  id="current" class="menuitem"><a href="student_report.php">Generate Report</a></li>
            
        </nav>
        <h1>Student Report</h1>
    </header>

    <section id="student-details">
        <!-- Include student details here -->
        <div class="profile-card">
            
            <i class="fa fa-user fa-5x" aria-hidden="true"></i>
            <h2>Gabriella Neema</h2>
            <p>Grade: LCB</p>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus et enim quis arcu semper tincidunt. Proin ut dolor vitae quam tincidunt luctus. Nam sed sapien et augue faucibus semper.</p>
        </div>    
        <!-- Add your content here -->
    </section>

    <section id="quiz-project">
        <!-- Include quiz and project assessment here -->
        <div class="cont">
            <div class="score_desc">
                <p>Quiz</p>
            </div>
            <div class="score_card">
                <div class="pie animate no-round" style="--p:80;--c:orange;"> 80%</div>
              
            </div>
        </div>
        <div class="cont">
            <div class="score_desc">
                <p>Final Project</p>
            </div>
            <div class="score_card">
                <div class="pie animate no-round" style="--p:80;--c:rgb(38, 0, 255);"> 50%</div>
             </div>
        </div>
        <!-- Add your content here -->
    </section>

    <section id="class-assessment">
        <!-- Include class assessment here -->
        <h2>Class Assessment</h2>
        <!-- Add your content here -->
        <div class="marks">
            <div class="cont_ass">
                <div class="score_desc">
                    <p>Attendence</p>
                </div>
                <div class="score_card">
                    <div class="pie animate no-round" style="--p:80;--c:rgb(0, 255, 149);"> 80%</div>
                  
                </div>
            </div>
            <div class="cont_ass">
                <div class="score_desc">
                    <p>Retention</p>
                </div>
                <div class="score_card">
                    <div class="pie animate no-round" style="--p:80;--c:rgb(255, 0, 200);"> 80%</div>
                  
                </div>
            </div>
            <div class="cont_ass">
                <div class="score_desc">
                    <p>Creativity</p>
                </div>
                <div class="score_card">
                    <div class="pie animate no-round" style="--p:80;--c:rgb(0, 255, 0);"> 80%</div>
                  
                </div>
            </div>
            <div class="cont_ass">
                <div class="score_desc">
                    <p>Application of knowledge</p>
                </div>
                <div class="score_card">
                    <div class="pie animate no-round" style="--p:80;--c:rgb(255, 0, 76);"> 80%</div>
                  
                </div>
            </div>
                <div class="cont_ass">
                        <div class="score_desc">
                            <p>Concentration in class</p>
                        </div>
                        <div class="score_card">
                            <div class="pie animate no-round" style="--p:80;--c:rgb(55, 0, 255);"> 80%</div>
                        
                        </div>
                </div>
                    <div class="cont_ass">
                        <div class="score_desc">
                            <p>Interest in coding</p>
                        </div>
                        <div class="score_card">
                            <div class="pie animate no-round" style="--p:80;--c:rgb(0, 255, 149);"> 80%</div>
                          
                        </div>
                    </div>
        </div>
        
    <footer>
        &copy; 2023 Code Academy Uganda
    </footer>

</body>
</html>
